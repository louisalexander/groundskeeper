...
inventory on a survey-accurate SVG of 12009 Blairmont Court) to a hosted, multi-device,
...
- **Bundled public-domain orthophoto** — the default *and* offline basemap (see §3.3). Sourced
  from VGIN (Virginia GIS) / Henrico County GIS / USDA NAIP, committed as a static app asset.
...
- **Source:** public-domain imagery — **VGIN (Virginia GIS)**, **Henrico County GIS**, or
...
- **Sourcing the orthophoto** (implementation task): obtain the best available public-domain
  georeferenced image of the lot from VGIN / Henrico County GIS / USDA NAIP, record its lat/lon
